...
{
    return GENUM2RVAL(GST_MESSAGE_TYPE(SELF(self)), GST_TYPE_MESSAGE_TYPE);
}

void
Init_gst_message(void)
{
    VALUE rb_cGstMessage;

    rb_cGstMessage = G_DEF_CLASS(GST_TYPE_MESSAGE, "Message", mGst);

    rb_define_method(rb_cGstMessage, "type", rb_gst_message_get_type, 0);

    G_DEF_CLASS(GST_TYPE_MESSAGE_TYPE, "Type", rb_cGstMessage);
