...
canvas_flags(r_self)
        VALUE r_self;
{
        return INT2NUM(get_art_canvas(r_self)->flags);
}
...
canvas_height(r_self)
        VALUE r_self;
{
        return INT2NUM(get_art_canvas(r_self)->height);
}
	...
	VALUE r_self;
{
	...
	cinfo.optimize_coding = TRUE;
        ...
        r_result = rb_str_buf_new(16384);
	cinfo.client_data = (void *) r_result;
        ...
        png_uint_32 length;
{
        ...
        rb_str_buf_cat(r_buffer, data, length);
}
...
canvas_width(r_self)
        VALUE r_self;
{
        return INT2NUM(get_art_canvas(r_self)->width);
}
