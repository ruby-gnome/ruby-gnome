...
gdkgc_s_new(self, win)
	 VALUE self, win;
{
  return make_gdkgc(gdk_gc_new(GDK_DRAWABLE(RVAL2GOBJ(win))));
}
...
gdkgc_set_clip_mask(self, mask)
	 VALUE self, mask;
{
  gdk_gc_set_clip_mask(get_gdkgc(self), GDK_BITMAP(RVAL2GOBJ(mask)));
  return mask;
}
...
gdkgc_set_stipple(self, stipple)
	 VALUE self, stipple;
{
  gdk_gc_set_stipple(get_gdkgc(self), GDK_PIXMAP(RVAL2GOBJ(stipple))); 
  return self;
}
...
gdkgc_set_tile(self, tile)
	 VALUE self, tile;
{
  gdk_gc_set_tile(get_gdkgc(self), GDK_PIXMAP(RVAL2GOBJ(tile))); 
  return self;
}
