...
class TestRegex < Test::Unit::TestCase
  ...
  sub_test_case "max_backref" do
    test "none" do
      a_regex = GLib::Regex.new("to?o", :compile_options => 0, :match_options => 0)
      assert_equal(a_regex.max_backref, 0)
    end
    test "one" do
      a_regex = GLib::Regex.new("(to(?)o)\\g1", :compile_options => 0, :match_options => 0)
  ...
  sub_test_case "capture_count" do
    ...
    test "three" do
      a_regex = GLib::Regex.new("((red|white) (king|queen))", :compile_options => 0, :match_options => 0)
  ...
  sub_test_case "has_cr_or_lf" do
    ...
    test "both" do
      a_regex = GLib::Regex.new("to\no", :compile_options => 0, :match_options => 0)
      assert(a_regex.has_cr_or_lf)
    end
    test "cr" do
      a_regex = GLib::Regex.new("to\rtiti", :compile_options => 0, :match_options => 0)
      assert(a_regex.has_cr_or_lf)
    end
  end
  ...
  sub_test_case "max_lookbehind" do
    ...
    test "three" do
      a_regex = GLib::Regex.new("(?<!foo)bar", :compile_options => 0, :match_options => 0)
  ...
  sub_test_case "string_number" do
    test "none" do
      a_regex = GLib::Regex.new("too", :compile_options => 0, :match_options => 0)
      assert_equal(a_regex.string_number("a_name"), -1)
    end
    test "one" do
      a_regex = GLib::Regex.new("(?<a_name>foo)bar", :compile_options => 0, :match_options => 0)
      assert_equal(a_regex.string_number("a_name"), 1)
    end
    test "two" do
      a_regex = GLib::Regex.new("(?<another_name>foo)(?<a_name>bar)", :compile_options => 0, :match_options => 0)
  ...
  def test_regex_split
    a_regex = GLib::Regex.new("\s", :compile_options => 0, :match_options => 0)
  ...
  def test_regex_replace_eval
    a_regex = GLib::Regex.new("1|2|3|4", :compile_options => 0, :match_options => 0)
  ...
  sub_test_case "match" do
    ...
    test "match longer" do
      a_regex = GLib::Regex.new("<.*>", :compile_options => 0, :match_options => 0)
