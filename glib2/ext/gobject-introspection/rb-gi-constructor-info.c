                    ...
                    GIArgument *value)
{
    ...
    if (gi_type_info_get_tag(&return_value_info) != GI_TYPE_TAG_INTERFACE) {
        rb_raise(rb_eRuntimeError, "TODO: returned value isn't interface");
    }

    interface_info = gi_type_info_get_interface(&return_value_info);
    ...
    if (GI_IS_OBJECT_INFO(interface_info)) {
      ...
      if (G_IS_OBJECT(value->v_pointer)) {
          ...
          switch (gi_callable_info_get_caller_owns(callable_info)) {
...
rg_invoke(VALUE self, VALUE rb_receiver, VALUE rb_arguments)
{
    ...
    /* TODO: use rb_protect */
    rb_gi_callable_info_invoke_raw(callable_info,
                                   self,
                                   G_TYPE_NONE,
                                   Qnil,
                                   rb_arguments,
