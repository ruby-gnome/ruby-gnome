...
rg_s_default(G_GNUC_UNUSED VALUE klass)
{
    ...
    return GOBJ2RVAL(default_repository);
}
...
rg_prepend_search_path(VALUE self, VALUE rb_path)
{
    ...
    gi_repository_prepend_search_path(repository, RVAL2CSTR(rb_path));
    return self;
}
...
rg_get_dependencies(VALUE self, VALUE rb_namespace)
{
    ...
    repository = SELF(self);
    namespace_ = RVAL2CSTR(rb_namespace);
    dependencies = gi_repository_get_dependencies(repository, namespace_, NULL);
    if (!dependencies) {
        return Qnil;
    }
...
rg_loaded_namespaces(VALUE self)
{
    ...
    repository = SELF(self);
    namespaces = gi_repository_get_loaded_namespaces(repository, NULL);
...
rb_gi_repository_init(VALUE rb_mGI)
{
    ...
    RG_TARGET_NAMESPACE = G_DEF_CLASS(GI_TYPE_REPOSITORY, "Repository", rb_mGI);
