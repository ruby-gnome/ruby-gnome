                             ...
                             GITypeInfo *type_info)
{
    ...
    if (type->info) {
        type->pointer_p = gi_type_info_is_pointer(type->info);
        type->tag = gi_type_info_get_tag(type->info);
    }
    if (type->tag == GI_TYPE_TAG_INTERFACE) {
        type->interface_info = gi_type_info_get_interface(type_info);
        if (GI_IS_REGISTERED_TYPE_INFO(type->interface_info)) {
            type->interface_gtype =
                gi_registered_type_info_get_g_type((GIRegisteredTypeInfo *)type->interface_info);
        }
    }
}
...
rb_gi_arg_metadata_type_clear(RBGIArgMetadataType *type)
{
    if (type->interface_info) {
        gi_base_info_unref(type->interface_info);
    }
    if (type->info) {
        gi_base_info_unref(type->info);
    }
}
                                  ...
                                  GITypeInfo *type_info)
{
    ...
    switch (metadata->type.tag) {
      case GI_TYPE_TAG_ARRAY:
        ...
        metadata->array_type = gi_type_info_get_array_type(type_info);
...
{
    metadata->struct_info = gi_struct_info_get_field(struct_info, GPOINTER_TO_UINT(struct_memory));
...
rb_gi_arg_metadata_new(GICallableInfo *callable_info, gint i)
{
    ...
    metadata->name = gi_base_info_get_name((GIBaseInfo*) arg_info);
    metadata->struct_info = NULL;
    metadata->struct_memory = NULL;
    metadata->index = i;

    rb_gi_arg_metadata_init_type_info(metadata,
                                      gi_arg_info_get_type_info(arg_info));

    metadata->scope_type = gi_arg_info_get_scope(arg_info);
    metadata->direction = gi_arg_info_get_direction(arg_info);
    metadata->transfer = gi_arg_info_get_ownership_transfer(arg_info);
    metadata->callback_p = (metadata->scope_type != GI_SCOPE_TYPE_INVALID);
    metadata->may_be_null_p = gi_arg_info_may_be_null(arg_info);
    metadata->caller_allocates_p = gi_arg_info_is_caller_allocates(arg_info);
...
rb_gi_arguments_allocate(RBGIArguments *args)
{
    ...
    gint n_args = gi_callable_info_get_n_args(args->info);
    gint i;
    for (i = 0; i < n_args; i++) {
...
rb_gi_arguments_fill_metadata_callback(RBGIArguments *args)
{
    ...
    for (i = 0; i < metadata->len; i++) {
        ...
        closure_index = gi_arg_info_get_closure_index(arg_info, NULL);
        if (closure_index != -1) {
        ...
        destroy_index = gi_arg_info_get_destroy_index(arg_info, NULL);
        if (destroy_index != -1) {
...
rb_gi_arguments_fill_metadata_array(RBGIArguments *args)
{
    ...
    for (i = 0; i < metadata->len; i++) {
        ...
        GITypeInfo *type_info;
        unsigned int *array_length_index = NULL;
        ...
        gi_type_info_get_array_length_index(type_info, array_length_index);
...
{
    GICallableInfo *info = args->info;
    GPtrArray *metadata = args->metadata;
    GITypeInfo return_type_info;
    RBGIArgMetadata *array_length_metadata;
    unsigned int *array_length_index = NULL;

    gi_callable_info_load_return_type(info, &return_type_info);
    if (gi_type_info_get_tag(&return_type_info) != GI_TYPE_TAG_ARRAY) {
        return;
    }

    gi_type_info_get_array_length_index(&return_type_info, array_length_index);
    if (array_length_index == NULL) {
        return;
    }
...
rb_gi_arguments_gobject_based_p(RBGIArguments *args)
{
    ...
    container_info = gi_base_info_get_container(GI_BASE_INFO(args->info));
    if (GI_IS_STRUCT_INFO(container_info)) {
        return TRUE;
    }

    registered_type_info = (GIRegisteredTypeInfo *)container_info;
    if (gi_registered_type_info_get_type_init_function_name(registered_type_info)) {
        return TRUE;
    }
                     ...
                     void **raw_args)
{
    args->info = info;
    if (info) {
        args->namespace = gi_base_info_get_namespace(GI_BASE_INFO(info));
        if (GI_IS_FUNCTION_INFO(info)) {
            args->name = gi_function_info_get_symbol((GIFunctionInfo *)info);
        } else {
            args->name = gi_base_info_get_name(GI_BASE_INFO(info));
        }
    } else {
        args->namespace = NULL;
        args->name = NULL;
...
rb_gi_arguments_convert_arg_interface_body(VALUE user_data)
{
    ...
    if (GI_IS_OBJECT_INFO(info) || GI_IS_INTERFACE_INFO(info)) {
        if (!G_IS_OBJECT(data->arg->v_pointer)) {
            ...
            ref = gi_object_info_get_ref_function_pointer(
                GI_OBJECT_INFO(data->arg_metadata->type.interface_info));
            if (ref)
...
rb_gi_arguments_convert_arg_interface_ensure(VALUE user_data)
{
    ...
    if (data->interface_info) {
        gi_base_info_unref(data->interface_info);
    }
                                      ...
                                      gboolean duplicate)
{
    ...
    data.interface_info = GI_INTERFACE_INFO(gi_type_info_get_interface(arg_metadata->type_info));
...
typedef struct {
    ...
    GIBaseInfo *interface_type_info;
} ArrayLikeToRubyData;
...
rb_gi_arguments_convert_arg_array_like_ensure(VALUE user_data)
{
    ...
    if (data->element_type_info) {
        gi_base_info_unref(data->element_type_info);
    }
    if (data->interface_type_info) {
        gi_base_info_unref(data->interface_type_info);
    }
    ...
    switch (metadata->type.tag) {
      ...
      case GI_TYPE_TAG_BOOLEAN:
        rb_raise(rb_eNotImpError,
                 ...
                 gi_type_tag_to_string(metadata->type.tag));
        return -1;
...
rb_gi_arguments_convert_arg_array_body_get_length(ArrayLikeToRubyData *data)
{
    ...
    unsigned int *out_length_index = NULL;
    gi_type_info_get_array_length_index(type_info, out_length_index);
    if (out_length_index == NULL) {
        return -1;
    }

    // TODO: I don't understand this. Fix it
    // if (!data->args->metadata && data->arg_metadata->struct_info) {
    //     GIFieldInfo *field_info =
    //         gi_struct_info_get_field(data->arg_metadata->struct_info,
    //                                  *out_length_index);
    //     GIArgument value;
    //     gi_field_info_get_field(data->arg_metadata->struct_info,
    //                            data->arg_metadata->struct_memory,
    //                            &value);
    //     RBGIArgMetadata length_metadata;
    //     GITypeInfo *length_type_info = gi_field_info_get_type_info(data->arg_metadata->struct_info);
    ...
    RBGIArgMetadata *length_metadata =
        g_ptr_array_index(data->args->metadata, *out_length_index);
    ...
    const char *array_c_type)
{
    ...
    data->interface_type_info =
        gi_type_info_get_interface(data->element_type_info);
    GType gtype = gi_registered_type_info_get_g_type((GIRegisteredTypeInfo *)data->interface_type_info);
    if (GI_IS_FUNCTION_INFO(data->interface_type_info) || GI_IS_CALLBACK_INFO(data->interface_type_info)) {
        rb_raise(rb_eNotImpError,
                 "TODO: GIArgument(array)[c][%s][interface(%s)](%s) -> Ruby",
                 array_c_type,
                 gi_base_info_get_name(data->interface_type_info),
                 g_type_name(gtype));
        return Qnil;
    }

    if (GI_IS_STRUCT_INFO(data->interface_type_info)) {
        if (gtype == G_TYPE_NONE) {
            VALUE rb_arg = rb_ary_new_capa(length);
            gint64 i;
            for (i = 0; i < length; i++) {
                rb_ary_push(rb_arg,
                            rb_gi_struct_info_to_ruby(GI_STRUCT_INFO(data->interface_type_info),
                                                      (gpointer)elements[i],
                                                      TRUE));
            }
            return rb_arg;
            ...
            gsize struct_size =
                gi_struct_info_get_size(GI_STRUCT_INFO(data->interface_type_info));
            VALUE rb_arg = rb_ary_new_capa(length);
            gint64 i;
            for (i = 0; i < length; i++) {
    ...
    if (GI_IS_ENUM_INFO(data->interface_type_info) || GI_IS_FLAGS_INFO(data->interface_type_info) || G_TYPE_IS_BOXED(gtype)) {
        rb_raise(rb_eNotImpError,
                 "TODO: GIArgument(array)[c][%s][interface(%s)](%s) -> Ruby",
                 array_c_type,
                 gi_base_info_get_name(data->interface_type_info),
    ...
    if (GI_IS_OBJECT_INFO(data->interface_type_info)) {
        VALUE rb_arg = rb_ary_new_capa(length);
        gint64 i;
        for (i = 0; i < length; i++) {
    ...
    switch (data->element_type_tag) {
      case GI_TYPE_TAG_VOID:
        rb_raise(rb_eNotImpError,
                 ...
                 array_c_type,
                 gi_type_tag_to_string(data->element_type_tag));
        return Qnil;
      case GI_TYPE_TAG_BOOLEAN:
        {
                                         ...
                                         gint64 length)
{
    ...
    GITypeInfo *type_info = data->arg_metadata->type_info;
    gi_type_info_get_array_fixed_size(type_info, fixed_size);
    gboolean zero_terminated_p = gi_type_info_is_zero_terminated(type_info);
                                  ...
                                  RBGIArgMetadata *arg_metadata)
{
    ...
    data.element_type_info =
        gi_type_info_get_param_type(arg_metadata->type_info, 0);
    data.element_type_tag = gi_type_info_get_tag(data.element_type_info);
...
rb_gi_arguments_convert_arg_glist_body_interface(ArrayLikeToRubyData *data)
{
    ...
    if (GI_IS_FLAGS_INFO(data->interface_type_info) || GI_IS_OBJECT_INFO(data->interface_type_info) || GI_IS_INTERFACE_INFO(data->interface_type_info)) {
...
rb_gi_arguments_convert_arg_gslist_body_interface(ArrayLikeToRubyData *data)
{
    ...
    if (GI_IS_FUNCTION_INFO(data->interface_type_info) || GI_IS_CALLABLE_INFO(data->interface_type_info)) {
        rb_raise(rb_eNotImpError,
    ...
    if (
        GI_IS_FLAGS_INFO(data->interface_type_info) || GI_IS_OBJECT_INFO(data->interface_type_info) ||
        GI_IS_INTERFACE_INFO(data->interface_type_info) || GI_IS_ENUM_INFO(data->interface_type_info)
      ) {
    ...
    switch (data->key_metadata->type.tag) {
      ...
      case GI_TYPE_TAG_GTYPE:
        rb_raise(rb_eNotImpError,
                 ...
                 gi_type_tag_to_string(data->key_metadata->type.tag),
                 gi_type_tag_to_string(data->value_metadata->type.tag));
        break;
      case GI_TYPE_TAG_UTF8:
...
rb_gi_arguments_convert_arg(RBGIArguments *args,
                            ...
                            gboolean duplicate)
{
    GITypeTag type_tag = gi_type_info_get_tag(arg_metadata->type_info);
    switch (type_tag) {
      case GI_TYPE_TAG_VOID:
        if (gi_type_info_is_pointer(arg_metadata->type_info)) {
    ...
    switch (element_type_tag) {
      case GI_TYPE_TAG_VOID:
        rb_raise(rb_eNotImpError,
                 ...
                 gi_type_tag_to_string(element_type_tag));
        break;
      case GI_TYPE_TAG_BOOLEAN:
      case GI_TYPE_TAG_INT8:
      case GI_TYPE_TAG_UINT8:
    ...
    ReturnValueToRubyData *data)
{
    ...
    GIBaseInfo* type = data->metadata->type.interface_info;

    if (GI_IS_FUNCTION_INFO(type) || GI_IS_CALLBACK_INFO(type)) {
        rb_raise(rb_eNotImpError,
                 "TODO: free GIArgument(interface(%s))(%s) everything",
                 gi_base_info_get_name(type),
                 g_type_name(gtype));
        return;
    }

    if (GI_IS_STRUCT_INFO(type)) {
    ...
    if (GI_IS_ENUM_INFO(type) || GI_IS_FLAGS_INFO(type) || G_TYPE_IS_BOXED(gtype)) {
        rb_raise(rb_eNotImpError,
                 "TODO: free GIArgument(interface(%s))(%s) everything",
                 gi_base_info_get_name(type),
                 g_type_name(gtype));
    ...
    if (GI_IS_OBJECT_INFO(type)) {
      ...
      if (G_IS_OBJECT(object)) {
          ...
          GIObjectInfoUnrefFunction unref = gi_object_info_get_unref_function_pointer(GI_OBJECT_INFO(data->metadata->type.interface_info));
          if (unref) {
              unref(object);
          }
      }
    ...
    if (GI_IS_INTERFACE_INFO(type)) {
      g_object_unref(data->value->v_pointer);
      return;
    }

    if (GI_IS_CONSTANT_INFO(type)) {
        rb_raise(rb_eNotImpError,
                 "TODO: free GIArgument(interface(%s))(%s) everything",
                 gi_base_info_get_name(type),
                 g_type_name(gtype));
    ...
    ReturnValueToRubyData *data)
{
    ...
    if (GI_IS_ENUM_INFO(type) || GI_IS_FLAGS_INFO(type)) {
        rb_raise(rb_eNotImpError,
                 "TODO: free GIArgument(GList)[interface(%s)](%s) everything",
                 gi_base_info_get_name(type),
                 g_type_name(gtype));
    ...
    if (GI_IS_INTERFACE_INFO(type) || GI_IS_OBJECT_INFO(type)) {
    ...
    ReturnValueToRubyData *data)
{
    ...
    if (GI_IS_OBJECT_INFO(type) || GI_IS_INTERFACE_INFO(type)) {
...
rb_gi_arguments_convert_return_value_ensure_body(VALUE user_data)
{
    ...
    switch (gi_callable_info_get_caller_owns(data->args->info)) {
...
{
    if (gi_callable_info_may_return_null(args->info) &&
        !return_value->v_pointer) {
        GITypeInfo return_value_info;
        gi_callable_info_load_return_type(args->info, &return_value_info);
        GITypeTag return_value_tag = gi_type_info_get_tag(&return_value_info);
    ...
    RBGIArgMetadata metadata;
    GITypeInfo *return_value_info = gi_callable_info_get_return_type(args->info);
...
rb_gi_arguments_fill_raw_out_gerror(RBGIArguments *args,
                                    VALUE rb_error)
{
    if (!gi_callable_info_can_throw_gerror(args->info)) {
        return;
    }
                                          ...
                                          gboolean is_return_value)
{
    ...
    GType gtype = gi_registered_type_info_get_g_type(GI_REGISTERED_TYPE_INFO(interface_info));

    if (GI_IS_FUNCTION_INFO(interface_info) || GI_IS_CALLBACK_INFO(interface_info)) {
        rb_raise(rb_eNotImpError,
                 "TODO: %s::%s: out raw result(interface): <%s>",
                 args->namespace,
                 args->name,
    ...
    if (GI_IS_FLAGS_INFO(interface_info)) {
        rb_raise(rb_eNotImpError,
                 "TODO: %s::%s: out raw result(interface): <%s>",
                 gi_base_info_get_namespace(GI_BASE_INFO(args->info)),
                 gi_base_info_get_name(GI_BASE_INFO(args->info)),
                 gi_base_info_get_name(interface_info));
        return;
    }

    if (GI_IS_OBJECT_INFO(interface_info) || GI_IS_INTERFACE_INFO(interface_info)) {
    ...
    gboolean is_return_value)
{
    ...
    interface_info = gi_type_info_get_interface(element_type_info);
    interface_name = gi_base_info_get_name(interface_info);
    gtype = gi_registered_type_info_get_g_type(GI_REGISTERED_TYPE_INFO(interface_info));

    if (
        GI_IS_FUNCTION_INFO(interface_info) || GI_IS_CALLBACK_INFO(interface_info) || GI_IS_STRUCT_INFO(interface_info) ||
        GI_IS_ENUM_INFO(interface_info) || GI_IS_FLAGS_INFO(interface_info) || G_TYPE_IS_BOXED(gtype)
    ) {
      gi_base_info_unref(interface_info);
                                      ...
                                      gboolean is_return_value)
{
    ...
    element_type_info = gi_type_info_get_param_type(type_info, 0);
    element_type_tag = gi_type_info_get_tag(element_type_info);
                                 ...
                                 gpointer raw_return_value)
{
    ...
    return_type_info = gi_callable_info_get_return_type(args->info);
    return_type_tag = gi_type_info_get_tag(return_type_info);
    if (return_type_tag != GI_TYPE_TAG_VOID) {
        GITransfer transfer = gi_callable_info_get_caller_owns(args->info);
        gboolean may_return_null = gi_callable_info_may_return_null(args->info);
    ...
    gi_base_info_unref(return_type_info);
    ...
    for (i = 0; i < args->metadata->len; i++) {
        ...
        argument = &g_array_index(args->out_args,
                                  GIArgument,
                                  metadata->out_arg_index);
        type_info = gi_arg_info_get_type_info(&(metadata->arg_info));

        transfer = gi_arg_info_get_ownership_transfer(&(metadata->arg_info));
