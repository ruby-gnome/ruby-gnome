...
rg_n_fields(VALUE self)
{
    ...
    info = SELF(self);
    return INT2NUM(gi_union_info_get_n_fields(info));
}
...
rg_get_field(VALUE self, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_union_info_get_field(info, n));
}
...
rg_get_field_value(VALUE self, VALUE rb_union, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    field_info = gi_union_info_get_field(info, n);
    gtype = gi_registered_type_info_get_g_type(GI_REGISTERED_TYPE_INFO(field_info));
...
rg_set_field_value(VALUE self, VALUE rb_union, VALUE rb_n, VALUE rb_value)
{
    ...
    registered_type_info = (GIRegisteredTypeInfo *)info;

    gtype = gi_registered_type_info_get_g_type(registered_type_info);
...
rg_n_methods(VALUE self)
{
    ...
    info = SELF(self);
    return INT2NUM(gi_union_info_get_n_methods(info));
}
...
rg_get_method(VALUE self, VALUE rb_n_or_name)
{
    ...
    if (RB_TYPE_P(rb_n_or_name, T_FIXNUM)) {
        ...
        function_info = gi_union_info_get_method(info, n);
    } else {
        const gchar *name;
        name = RVAL2CSTR(rb_n_or_name);
        function_info = gi_union_info_find_method(info, name);
    }
...
rg_discriminated_p(VALUE self)
{
    ...
    info = SELF(self);
    return CBOOL2RVAL(gi_union_info_is_discriminated(info));
}
...
rg_discriminator_type(VALUE self)
{
    ...
    info = SELF(self);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_union_info_get_discriminator_type(info));
}
...
rg_get_discriminator(VALUE self, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_union_info_get_discriminator(info, n));
}
...
rg_size(VALUE self)
{
    ...
    info = SELF(self);
    return ULONG2NUM(gi_union_info_get_size(info));
}
...
rg_alignment(VALUE self)
{
    ...
    info = SELF(self);
    return ULONG2NUM(gi_union_info_get_alignment(info));
}
