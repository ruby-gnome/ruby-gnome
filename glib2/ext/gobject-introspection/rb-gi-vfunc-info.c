...
rg_flags(VALUE self)
{
    ...
    info = SELF(self);
    return GI_VFUNC_INFO_FLAGS2RVAL(gi_vfunc_info_get_flags(info));
}
...
rg_offset(VALUE self)
{
    ...
    info = SELF(self);
    return INT2NUM(gi_vfunc_info_get_offset(info));
}
...
rg_signal(VALUE self)
{
    ...
    info = SELF(self);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_vfunc_info_get_signal(info));
}
...
rg_invoker(VALUE self)
{
    ...
    info = SELF(self);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_vfunc_info_get_invoker(info));
}
          ...
          VALUE rb_arguments)
{
    ...
    /* TODO: use rb_protect */
    VALUE rb_out_args = rb_gi_callable_info_invoke_raw(callable_info,
                                                       self,
                                                       implementor_gtype,
                                                       rb_receiver,
                                                       rb_arguments,
