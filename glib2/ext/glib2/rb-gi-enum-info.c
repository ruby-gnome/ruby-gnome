...
rg_n_values(VALUE self)
{
    ...
    info = SELF(self);
    return INT2NUM(gi_enum_info_get_n_values(info));
}
...
rg_get_value(VALUE self, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_enum_info_get_value(info, n));
}
...
rg_values(VALUE self)
{
    ...
    n = gi_enum_info_get_n_values(info);
    for (i = 0; i < n; i++) {
...
rg_n_methods(VALUE self)
{
    ...
    info = SELF(self);
    return INT2NUM(gi_enum_info_get_n_methods(info));
}
...
rg_get_method(VALUE self, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_enum_info_get_method(info, n));
}
...
rg_methods(VALUE self)
{
    ...
    n = gi_enum_info_get_n_methods(info);
    for (i = 0; i < n; i++) {
        GIFunctionInfo *function_info;
        function_info = gi_enum_info_get_method(info, i);
...
rg_storage_type(VALUE self)
{
    ...
    info = SELF(self);
    return GI_TYPE_TAG2RVAL(gi_enum_info_get_storage_type(info));
}
...
rg_error_domain(VALUE self)
{
    ...
    info = SELF(self);
    return CSTR2RVAL(gi_enum_info_get_error_domain(info));
}
