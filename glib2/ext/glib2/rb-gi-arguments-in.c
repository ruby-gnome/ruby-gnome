                                          ...
                                          RBGIArgMetadata *metadata)
{
    ...
    if (!rb_block_given_p() && gi_arg_info_may_be_null(arg_info)) {
    ...
    if (callback_function) {
        ...
        GITypeInfo *type_info = gi_arg_info_get_type_info(arg_info);
        GIBaseInfo *callback_info = gi_type_info_get_interface(type_info);
        callback = rb_gi_callback_new(GI_CALLBACK_INFO(callback_info), NULL);
        gi_base_info_unref(callback_info);
                                      ...
                                      RBGIArgMetadata *metadata)
{
    ...
    if (!gi_type_info_is_pointer(metadata->type.info)) {
        *target = NULL;
        return;
    }
    ...
    switch (length_metadata->type.tag) {
      ...
      case GI_TYPE_TAG_BOOLEAN:
        rb_raise(rb_eNotImpError,
                 ...
                 gi_type_tag_to_string(length_metadata->type.tag));
        break;
                                                          ...
                                                          VALUE rb_array)
{
    ...
    if (metadata->element_type.pointer_p) {
        if (metadata->transfer != GI_TRANSFER_NOTHING) {
            rb_raise(rb_eNotImpError,
                     "TODO: [%s::%s] %s "
                     "Ruby -> GIArgument(array/%s)[interface(%s)](%s)",
                     args->name,
                     metadata->name,
        ...
        gsize struct_size = gi_struct_info_get_size(struct_info);
                                                 ...
                                                 gpointer user_data)
{
    ...
    switch (metadata->transfer) {
      ...
      case GI_TRANSFER_CONTAINER:
        ...
        rb_raise(rb_eNotImpError,
                 "TODO: %s Ruby -> GIArgument(array/%s)[interface(%s)]",
                 rb_gi_direction_to_string(metadata->direction),
                 rb_gi_array_type_to_string(metadata->array_type),
                 g_type_name(metadata->element_type.interface_gtype));
        break;
...
{
    if (
        GI_IS_INTERFACE_INFO(metadata->element_type.interface_info) ||
        GI_IS_CALLBACK_INFO(metadata->element_type.interface_info) ||
        GI_IS_FUNCTION_INFO(metadata->element_type.interface_info)
    ) {
        rb_raise(rb_eNotImpError,
                 "TODO: %s Ruby -> GIArgument(array/%s)[interface(%s)]",
                 rb_gi_direction_to_string(metadata->direction),
    ...
    if (GI_IS_STRUCT_INFO(metadata->element_type.interface_info)) {
    ...
    if (
        (GI_IS_REGISTERED_TYPE_INFO(metadata->element_type.interface_info) && 
            gi_registered_type_info_is_boxed(GI_REGISTERED_TYPE_INFO(metadata->element_type.interface_info))) ||
        GI_IS_ENUM_INFO(metadata->element_type.interface_info) ||
        GI_IS_FLAGS_INFO(metadata->element_type.interface_info)
    ) {
        rb_raise(rb_eNotImpError,
                 "TODO: %s Ruby -> GIArgument(array/%s)[interface(%s)]",
                 rb_gi_direction_to_string(metadata->direction),
    ...
    if (GI_IS_OBJECT_INFO(metadata->element_type.interface_info)) {
    ...
    if (
        GI_IS_INTERFACE_INFO(metadata->element_type.interface_info) ||
        GI_IS_CONSTANT_INFO(metadata->element_type.interface_info) ||
        GI_IS_UNION_INFO(metadata->element_type.interface_info) ||
        GI_IS_VALUE_INFO(metadata->element_type.interface_info) ||
        GI_IS_SIGNAL_INFO(metadata->element_type.interface_info) ||
        GI_IS_VFUNC_INFO(metadata->element_type.interface_info) ||
        GI_IS_PROPERTY_INFO(metadata->element_type.interface_info) ||
        GI_IS_FIELD_INFO(metadata->element_type.interface_info) ||
        GI_IS_ARG_INFO(metadata->element_type.interface_info) ||
        GI_IS_TYPE_INFO(metadata->element_type.interface_info) ||
        GI_IS_UNRESOLVED_INFO(metadata->element_type.interface_info)
    ) {
        rb_raise(rb_eNotImpError,
                 "TODO: %s Ruby -> GIArgument(array/%s)[interface(%s)]",
                 rb_gi_direction_to_string(metadata->direction),
    ...
    switch (metadata->element_type.tag) {
      case GI_TYPE_TAG_VOID:
        rb_raise(rb_eNotImpError,
                 ...
                 rb_gi_direction_to_string(metadata->direction),
                 rb_gi_array_type_to_string(metadata->array_type),
                 gi_type_tag_to_string(metadata->element_type.tag),
                 rb_gi_transfer_to_string(metadata->transfer));
        break;
      case GI_TYPE_TAG_BOOLEAN:
...
{
    if (
        GI_IS_FUNCTION_INFO(metadata->type.interface_info) ||
        GI_IS_CALLABLE_INFO(metadata->type.interface_info)
    ) {
        rb_raise(rb_eNotImpError,
                 ...
                 g_type_name(metadata->type.interface_gtype),
                 rb_gi_transfer_to_string(metadata->transfer));
    } else if (GI_IS_STRUCT_INFO(metadata->type.interface_info)) {
        ...
        rb_raise(rb_eNotImpError,
                 ...
                 g_type_name(metadata->type.interface_gtype),
                 rb_gi_transfer_to_string(metadata->transfer));
    }else if (GI_IS_ENUM_INFO(metadata->type.interface_info)) {
        if (metadata->type.interface_gtype == G_TYPE_NONE) {
    ...
    } else if (GI_IS_FLAGS_INFO(metadata->type.interface_info)) {
        if (metadata->type.interface_gtype == G_TYPE_NONE) {
            metadata->in_arg->v_int32 = NUM2INT(metadata->rb_arg);
        } else {
            metadata->in_arg->v_int32 =
        ...
        GI_IS_OBJECT_INFO(metadata->type.interface_info) ||
        GI_IS_INTERFACE_INFO(metadata->type.interface_info)
    ) {
    ...
    } else if (GI_IS_CONSTANT_INFO(metadata->type.interface_info)) {
        rb_raise(rb_eNotImpError,
                 "TODO: [%s][%s](%s) %s "
                 "Ruby -> GIArgument(interface)[%s][%s]",
                 args->namespace,
                 ...
                 g_type_name(metadata->type.interface_gtype),
                 rb_gi_transfer_to_string(metadata->transfer));
    } else if (GI_IS_UNION_INFO(metadata->type.interface_info)) {
        if (metadata->transfer != GI_TRANSFER_NOTHING) {
            rb_raise(rb_eNotImpError,
                     "TODO: [%s] %s Ruby -> GIArgument(interface)[%s][%s]",
                     metadata->name,
        ...
        rb_raise(rb_eNotImpError,
                 "TODO: [%s] %s Ruby -> GIArgument(interface)[%s]",
                 metadata->name,
                 rb_gi_direction_to_string(metadata->direction),
                 rb_gi_transfer_to_string(metadata->transfer));
    }
    ...
    switch (metadata->key_type.tag) {
      ...
      case GI_TYPE_TAG_GTYPE:
        rb_raise(rb_eNotImpError,
                 ...
                 gi_type_tag_to_string(metadata->key_type.tag));
        break;
      case GI_TYPE_TAG_UTF8:
    ...
    switch (metadata->value_type.tag) {
      ...
      case GI_TYPE_TAG_GTYPE:
        rb_raise(rb_eNotImpError,
                 ...
                 gi_type_tag_to_string(metadata->value_type.tag));
        break;
      case GI_TYPE_TAG_UTF8:
      ...
      case GI_TYPE_TAG_INTERFACE:
        if (
            GI_IS_FUNCTION_INFO(metadata->value_type.interface_info) ||
            GI_IS_CALLBACK_INFO(metadata->value_type.interface_info) ||
            GI_IS_STRUCT_INFO(metadata->value_type.interface_info) ||
            (GI_IS_REGISTERED_TYPE_INFO(metadata->element_type.interface_info) && 
            gi_registered_type_info_is_boxed(GI_REGISTERED_TYPE_INFO(metadata->element_type.interface_info)))
        ) {
            rb_raise(rb_eNotImpError,
                     ...
                     gi_type_tag_to_string(metadata->value_type.tag),
                     g_type_name(metadata->value_type.interface_gtype));
        } else if (GI_IS_ENUM_INFO(metadata->value_type.interface_info)) {
                                 ...
                                 RBGIArgMetadata *metadata)
{
    ...
    if (NIL_P(metadata->rb_arg)) {
        ...
        klass = gi_base_info_get_container(GI_BASE_INFO(args->info));
        if (metadata->rb_arg_index == 1) {
            suffix = "st";
        ...
        rb_raise(rb_eArgError,
                 ...
                 klass ? "::" : "",
                 klass ? gi_base_info_get_name(klass) : "",
                 klass ? "#" : ".",
                                          ...
                                          RBGIArgMetadata *metadata)
{
    if (
        GI_IS_FUNCTION_INFO(metadata->type.interface_info) ||
        GI_IS_CALLBACK_INFO(metadata->type.interface_info)
    ) { 
        rb_raise(rb_eNotImpError,
                 ...
                 gi_base_info_get_name(GI_BASE_INFO(args->info)),
                 g_type_name(metadata->type.interface_gtype),
                 gi_base_info_get_name(metadata->type.interface_info));
        return;
    }

    if (GI_IS_STRUCT_INFO(metadata->type.interface_info)) {
    ...
    if (GI_IS_ENUM_INFO(metadata->element_type.interface_info)) {
        metadata->in_arg->v_int32 = *((gint32 *)(args->raw_args[metadata->index]));
        return;
    }

    if (GI_IS_TYPE_INFO(metadata->element_type.interface_info)) {
        metadata->in_arg->v_int32 = *((gint32 *)(args->raw_args[metadata->index]));
        return;
    }

    if (
        GI_IS_OBJECT_INFO(metadata->element_type.interface_info) ||
        GI_IS_INTERFACE_INFO(metadata->element_type.interface_info)
    ) {
        metadata->in_arg->v_pointer = *((gpointer *)(args->raw_args[metadata->index]));
        return;
    ...
    if (GI_IS_CONSTANT_INFO(metadata->element_type.interface_info)) {
        rb_raise(rb_eNotImpError,
                 "TODO: %s::%s: raw argument -> GIArgument(interface)[%s]: <%s>",
                 gi_base_info_get_namespace(GI_BASE_INFO(args->info)),
                 gi_base_info_get_name(GI_BASE_INFO(args->info)),
