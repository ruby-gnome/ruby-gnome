...
rg_symbol(VALUE self)
{
    ...
    info = SELF(self);
    return CSTR2RVAL(gi_function_info_get_symbol(info));
}
...
rg_flags(VALUE self)
{
    ...
    info = SELF(self);
    return GI_FUNCTION_INFO_FLAGS2RVAL(gi_function_info_get_flags(info));
}
...
rg_property(VALUE self)
{
    ...
    info = SELF(self);
    return GI_BASE_INFO2RVAL(gi_function_info_get_property(info));
}
...
rg_vfunc(VALUE self)
{
    ...
    info = SELF(self);
    return GI_BASE_INFO2RVAL(gi_function_info_get_vfunc(info));
}
...
rg_invoke(VALUE self, VALUE rb_arguments)
{
    ...
    if (NIL_P(rb_out_args)) {
        ...
        gi_callable_info_load_return_type(callable_info, &return_value_info);
        if (gi_type_info_get_tag(&return_value_info) != GI_TYPE_TAG_VOID) {
...
{
    RG_TARGET_NAMESPACE =
	G_DEF_CLASS_WITH_PARENT(gi_function_info_get_type (), "FunctionInfo", rb_mGI,
				rb_cGICallableInfo);
