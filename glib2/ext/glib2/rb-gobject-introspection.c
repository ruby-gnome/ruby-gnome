...
Init_gobject_introspection(void)
{
    ...
    VALUE gobject_introspection = rb_define_module_under(rbg_mGLib(), "GObjectIntrospection");

    rb_define_const(gobject_introspection, "BUILD_VERSION",
                    rb_ary_new3(3,
                                INT2FIX(GI_MAJOR_VERSION),
                                INT2FIX(GI_MINOR_VERSION),
                                INT2FIX(GI_MICRO_VERSION)));

    rb_gi_array_type_init(gobject_introspection);
    rb_gi_type_tag_init(gobject_introspection);
    rb_gi_base_info_init(gobject_introspection);
    rb_gi_repository_init(gobject_introspection);
    rb_gi_loader_init(gobject_introspection);

    rb_gi_callback_init(gobject_introspection);
}
