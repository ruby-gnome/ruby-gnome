                          ...
                          gboolean is_pointer)
{
    ...
    namespace = gi_base_info_get_namespace(base_info);
    name = gi_base_info_get_name(base_info);
    ...
    if (is_pointer) {
        ...
        object_size = gi_struct_info_get_size(info);
...
rg_n_fields(VALUE self)
{
    ...
    info = SELF(self);
    return INT2NUM(gi_struct_info_get_n_fields(info));
}
...
rg_get_field(VALUE self, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_struct_info_get_field(info, n));
}
...
rg_find_field(VALUE self, VALUE rb_name)
{
    ...
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_struct_info_find_field(info, name));
}
...
rg_get_field_value(VALUE self, VALUE rb_struct, VALUE rb_n)
{
    ...
    n = NUM2INT(rb_n);
    field_info = gi_struct_info_get_field(info, n);
...
rg_n_methods(VALUE self)
{
    ...
    info = SELF(self);
    return INT2NUM(gi_struct_info_get_n_methods(info));
}
...
rg_get_method(VALUE self, VALUE rb_n_or_name)
{
    ...
    if (RB_TYPE_P(rb_n_or_name, RUBY_T_FIXNUM)) {
        ...
        function_info = gi_struct_info_get_method(info, n);
    } else {
        const gchar *name;
        name = RVAL2CSTR(rb_n_or_name);
        function_info = gi_struct_info_find_method(info, name);
    }
...
rg_size(VALUE self)
{
    ...
    info = SELF(self);
    return UINT2NUM(gi_struct_info_get_size(info));
}
...
rg_alignment(VALUE self)
{
    ...
    info = SELF(self);
    return UINT2NUM(gi_struct_info_get_alignment(info));
}
...
rg_gtype_struct_p(VALUE self)
{
    ...
    info = SELF(self);
    return CBOOL2RVAL(gi_struct_info_is_gtype_struct(info));
}
...
rg_foreign_p(VALUE self)
{
    ...
    info = SELF(self);
    return CBOOL2RVAL(gi_struct_info_is_foreign(info));
}
