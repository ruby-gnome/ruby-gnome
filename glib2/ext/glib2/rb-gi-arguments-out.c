                                                      ...
                                                      gpointer user_data)
{
    ...
    switch (metadata->transfer) {
      ...
      default:
        rb_raise(rb_eNotImpError,
                 "TODO: [%s] %s free GIArgument(%s)[interface(%s)](%s)[%s]",
                 metadata->name,
                 rb_gi_direction_to_string(metadata->direction),
                 gi_type_tag_to_string(metadata->type.tag),
                 rb_gi_array_type_to_string(metadata->array_type),
                                                   ...
                                                   RBGIArgMetadata *metadata)
{
    ...
    if (GI_IS_STRUCT_INFO(metadata->element_type.interface_info)) {
        ...
        raw_struct_size =
            gi_struct_info_get_size(GI_STRUCT_INFO(metadata->element_type.interface_info));
    ...
    rb_raise(rb_eNotImpError,
                 "TODO: [%s] %s allocates "
                 "GIArgument(array)[array][interface(%s)]",
                 metadata->name,
                 rb_gi_direction_to_string(metadata->direction),
                 g_type_name(metadata->element_type.interface_gtype));
}
                                       ...
                                       RBGIArgMetadata *metadata)
{
    ...
    if (GI_IS_STRUCT_INFO(metadata->element_type.interface_info)) {
        ...
        if (metadata->type.pointer_p) {
            ...
            *struct_location = NULL;
            ...
            gsize struct_size =
                gi_struct_info_get_size(GI_STRUCT_INFO(metadata->type.interface_info));
    ...
    if (GI_IS_FLAGS_INFO(metadata->element_type.interface_info)) {
        {
    ...
    if (GI_IS_INTERFACE_INFO(metadata->element_type.interface_info)) {
        {
...
rb_gi_arguments_out_to_ruby(RBGIArguments *args)
{
    ...
    n_args = gi_callable_info_get_n_args(args->info);
    for (i = 0; i < n_args; i++) {
        RBGIArgMetadata *metadata;
