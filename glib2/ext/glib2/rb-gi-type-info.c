...
rg_get_param_type(VALUE self, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_type_info_get_param_type(info, n));
}
...
rg_interface(VALUE self)
{
    ...
    info = SELF(self);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_type_info_get_interface(info));
}
...
rg_array_length(VALUE self)
{
    ...
    info = SELF(self);
    gi_type_info_get_array_length_index(info, &out_length_index);
    return INT2NUM(out_length_index);
}
...
rg_array_fixed_size(VALUE self)
{
    ...
    info = SELF(self);
    gi_type_info_get_array_fixed_size(info, &out_size);
    return INT2NUM(out_size);
}
