...
{
    if (GI_IS_VFUNC_INFO(data->info)) {
        data->succeeded =
            gi_vfunc_info_invoke((GIVFuncInfo *)(data->info),
        ...
        data->succeeded =
            gi_function_info_invoke((GIFunctionInfo *)(data->info),
                                   (GIArgument *)((void *)(data->in_args->data)),
                                   data->in_args->len,
                                   (GIArgument *)((void *)(data->out_args->data)),
                                   data->out_args->len,
...
rg_return_type(VALUE self)
{
    ...
    info = SELF(self);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_callable_info_get_return_type(info));
}
...
rg_get_arg(VALUE self, VALUE rb_n)
{
    ...
    info = SELF(self);
    n = NUM2INT(rb_n);
    return GI_BASE_INFO2RVAL_WITH_UNREF(gi_callable_info_get_arg(info, n));
}
...
rb_gi_callable_info_init(VALUE rb_mGI, VALUE rb_cGIBaseInfo)
{
    ...
    RG_TARGET_NAMESPACE =
        G_DEF_CLASS_WITH_PARENT(gi_callable_info_get_type (), "CallableInfo", rb_mGI,
                                rb_cGIBaseInfo);
