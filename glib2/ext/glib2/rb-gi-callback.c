...
rb_gi_callback_invoke(VALUE user_data)
{
    ...
    if (state != 0) {
        ...
        if (!gi_callable_info_can_throw_gerror(data->args->info)) {
                           ...
                           void *data)
{
    ...
    rb_gi_arguments_init(&args,
                         GI_CALLABLE_INFO(callback->callback_info),
                         Qnil,
                         Qnil,
                         raw_args);
    {
                   ...
                   const gchar *method_name)
{
    ...
    gi_base_info_ref(callback->callback_info);
    callback->method_name = g_strdup(method_name);
    callback->closure =
        gi_callable_info_create_closure(GI_CALLABLE_INFO(callback->callback_info),
                                       &(callback->cif),
                                       rb_gi_ffi_closure_callback,
                                       callback);
    callback->closure_native_address =
        gi_callable_info_get_closure_native_address(GI_CALLABLE_INFO(callback->callback_info),
                                                   callback->closure);
    return callback;
}
...
{
    gi_callable_info_destroy_closure(GI_CALLABLE_INFO(callback->callback_info),
                                    callback->closure);
    g_free(callback->method_name);
    gi_base_info_unref(callback->callback_info);
    xfree(callback);
}
...
source_func_p(GIArgInfo *info)
{
    ...
    if (GI_IS_CALLBACK_INFO(interface_info)) {
        gi_base_info_unref(interface_info);
        return FALSE;
    }

    callback_info = (GICallableInfo *)interface_info;
    gi_callable_info_load_return_type(callback_info, &return_type_info);
    if (gi_type_info_get_tag(&return_type_info) != GI_TYPE_TAG_BOOLEAN) {
        gi_base_info_unref(interface_info);
        return FALSE;
    }

    if (gi_callable_info_get_n_args(callback_info) != 1) {
        gi_base_info_unref(interface_info);
        return FALSE;
    }

    gi_callable_info_load_arg(callback_info, 0, &first_arg_info);
    gi_arg_info_load_type_info(&first_arg_info, &first_arg_type_info);
    if (gi_type_info_get_tag(&first_arg_type_info) != GI_TYPE_TAG_VOID) {
        gi_base_info_unref(interface_info);
        return FALSE;
    }
